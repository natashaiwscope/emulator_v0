...
struct options
{
    char server_name[256];    /* hostname of the server (client only)     */
    char server_port[256];    /* port on which the ssl service runs       */
    int debug_level;            /* level of debugging                       */
    int authentication;         /* if authentication is required            */
    int mode;                   /* SSL/TLS (0) or STARTTLS (1)              */
    char user_name[256];      /* username to use for authentication       */
    char user_pwd[256];       /* password to use for authentication       */
    char mail_from[256];      /* E-Mail address to use as sender          */
    char mail_to[256];        /* E-Mail address to use as recipient       */
...
int setCredencial(const char *smtp_serverName,
                  ...
                  const char *userTo            )
{
    strcpy(opt.server_name,smtp_serverName);
    strcpy(opt.server_port,smtp_port);
    strcpy(opt.user_name  ,userName);
    strcpy(opt.user_pwd   ,userPass);
    strcpy(opt.mail_from  ,userFrom);
    strcpy(opt.mail_to    ,userTo);
}
